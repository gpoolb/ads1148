...
// Burn-out Detect Current Source Register  selection
...
// Multiplexer Selection - ADC Positive Input selection
...
// Multiplexer Selection - ADC Negative Input selection
...
// VBIAS—Bias Voltage Register output selection
...
// Internal Reference Control selection
...
// Reference Select Control selection
...
// System Monitor Control selection
...
//  extern double thermocoupleCoefficients [3][10];
//  extern uint16_t rangeThermocouple [4];
...
//	uint8_t ADS1148RegisterValues [] = {0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, \
										0x00, 0x00, 0x40, 0x00, 0xff, 0x00, 0x00, 0x00};  // Valores iniciales.
    const uint32_t SCLK       = 2000000;///< ADS1148 SCLK frequency: 2000000 Hz Maximum for ADS1148
	void writeRegister(uint8_t address, uint8_t value);
    uint8_t readRegister(uint8_t address);
    bool begin();
    bool isDataReady(void);
    byte * readADC_Array(bool isContinuous = false);
	void setReadingInternalTemperature();
	void setReadingThermocouple(uint8_t channel = ANALOG_CHANNEL_0);
	double readInternalTemperature(bool isContinuous = false);
	double readInputVoltage(bool isContinuous = false);
	double readInputVoltageIn_mV(bool isContinuous = false);
	double readInputThermocouple(bool isContinuous = false);
